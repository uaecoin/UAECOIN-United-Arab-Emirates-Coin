...
unsigned int nTargetSpacing = 3 * 60; //time block
unsigned int nStakeMinAge = 4 * 60 * 60; //min stake
unsigned int nStakeMaxAge = -1; // no max
unsigned int nModifierInterval = 10 * 60;

int nCoinbaseMaturity = 30;// +10
...
const string strMessageMagic = "UAECOIN Signed Message:\n";
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "UAECOIN", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "UNITED ARAB EMIRATES COIN - UAEC";
        CTransaction txNew;
        txNew.nTime = 1463476380;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
		...
		uint256 nhashMerkleRoot = uint256("0xd366d4fe7df4253c44cb2dc472684d434add55547bda7cda5bbe2e025dbade84");
